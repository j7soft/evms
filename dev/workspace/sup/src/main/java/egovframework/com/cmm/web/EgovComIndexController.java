...
public class EgovComIndexController implements ApplicationContextAware, InitializingBean {
	...
	public String index(ModelMap model,HttpServletRequest request) {
		...
		List<ContentVO> programlist = new ArrayList<>();
		List<ContentVO> bannerlist = new ArrayList<>();
		List<ContentVO> popuplist = new ArrayList<>();
		
		for(int i = 0 ; i < list.size(); i++){

			System.out.println(list.get(i).toString());
			if(list.get(i).getNTT_NO().trim().equals("popup")){
				popuplist.add(list.get(i));
			}else if(list.get(i).getNTT_NO().trim().equals("banner")){
				bannerlist.add(list.get(i));
			}else if(list.get(i).getNTT_NO().trim().equals("program")){
		...
		noticevo.setNTT_ID("notice");
		List<BoardVO> noticeList = egovUserService.boardSelect(noticevo);

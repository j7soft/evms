...
<script type="text/javascript">
	$(function() {
		...
		$(document).on("click", "#btn", function() {
			...
			var saveRadio = $(thisForm).find('input[name="REFLCT_AT"]:checked');
			if(saveRadio.length < 1) {
				alert("사용유무를 체크해주세요.");
				return;
			}
			...
			$(this).parents("form").ajaxForm({ //파일과 함께 보낼때는 ajaxForm을 사용
				url : "/admin/ContentInsert.do?NTT_NO=banner",
				type : "post",
...
<div id="file_list_div">
	<c:forEach begin="0" end="5" varStatus="i">
		<fieldset style="margin-bottom:100px;" class="con_banner">
			<legend id="lg">컨텐츠 관리 - 배너${i.index+1 }</legend>
			<form id="frm" name="frm" class="frm">
				<input type="hidden" name="BANNER_NM" value='banner${i.index+1}' />
				...
				<div>
					<label for="image">이미지</label> <input type="file" name="file"
						...
						height="100" width="430" /> <input type="hidden" name="b_file"
				...
				<div>
					...
					<span style="font-weight: 300; font-size: 0.85rem; margin-left: 43px; color: red;">1920px*455px 사이즈로 작업해주세요</span>
				</div>
			</form>
		</fieldset>
	</c:forEach>

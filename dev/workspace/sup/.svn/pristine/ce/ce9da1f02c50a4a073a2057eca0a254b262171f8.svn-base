...
<script type="text/javaScript" language="javascript">
	...
	******************************************************** */
	...
	function fnEgovSchdulTodate() {
		...
		location.href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.do' />?year=<%=sTodate.substring(0, 4)%>&month=<%=Integer.valueOf(sTodate.substring(4, 6)) - 1%>&day=<%=sTodate.substring(6, 8)%>
	&searchCondition=SCHDUL_SE&searchKeyword="
				+ document.deptSchdulManageVO.schdulSe.value;
...
<form name="deptSchdulManageVO" id="deptSchdulManageVO" action=""
	...
	<DIV id="content" style="width: 712px;">
			...
			<div class="date_view">
				<ul>
					<li><a href="javascript:fnEgovSchdulTodate();" class="today"><spring:message
								code="comCopSmtSdm.Navi.Today" /></a></li>
					<!-- 오늘 -->

					<li><a class="prev"
						href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.do' />?year=<%=iNowYear - 1%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay%>">prev</a></li>
					<li class="date"><%=iNowYear%> <spring:message
							code="comCopSmtSdm.Navi.Year" /></li>
					<!-- 년 -->
					<li><a class="next"
						href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.do' />?year=<%=iNowYear + 1%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay%>">next</a></li>
					...
					%>
					<li><a class="prev"
						href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.do' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth - 1%>&amp;day=<%=iNowDay%>">prev</a></li>
					<%
						}
					%>
					<li class="date"><%=iNowMonth + 1%> <spring:message
							code="comCopSmtSdm.Navi.Month" /></li>
					<!-- 월 -->
					<%
						if (iNowMonth < 11) {
					%>
					<li><a class="next"
						href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.do' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth + 1%>&amp;day=<%=iNowDay%>">next</a></li>
					<%
						}
					%>
					<li class="date">&nbsp;</li>
					...
					%>
					<li><a class="prev"
						href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.do' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay - 1%>">prev</a></li>
					<%
						}
					%>
					<li class="date"><%=iNowDay%> <spring:message
							code="comCopSmtSdm.Navi.Day" /></li>
					<!-- 일 -->
					<%
						if (iNowDay < iEndDay) {
					%>
					<li><a class="next"
						href="<c:url value='/cop/smt/sdm/EgovDeptSchdulManageDailyList.do' />?year=<%=iNowYear%>&amp;month=<%=iNowMonth%>&amp;day=<%=iNowDay + 1%>">next</a></li>
					<%
						}
					%>
		...
		<div class="week_calendar">
			<table class="title">
				<caption>
					<spring:message code="comCopSmtSdm.Gbn.Daily" />
					<spring:message code="comCopSmtSdm.title" />
				</caption>
				<!-- 일간 부서일정관리 -->
				<colgroup>
				...
				<thead>
					<tr>
						<th><spring:message code="comCopSmtSdm.Weekly.time" /></th>
						<!-- 시간 -->
						<th><spring:message code="comCopSmtSdm.Weekly.title" /></th>
						<!-- 제목 -->
						<th><spring:message code="comCopSmtSdm.Weekly.chargeName" /></th>
						<!-- 담당자 -->
					</tr>
				<tbody>
					<%
					...
					<tr>
						<td height="50px">
							<%
								out.print("<a href=\"JavaScript:fn_egov_detail_DeptSchdulManage('"
						...
						<td><%=(String) egovMap.get("schdulChargerName")%></td>
					</tr>
					...
					<c:if test="${fn:length(resultList) == 0}">
						<tr>
							<td bgcolor="#FFFFFFFF" nowrap colspan="3" align="center"
								style="padding-center: 2px; font-size: 11px;" height="30px"><spring:message

...
<c:forEach items="${list}" var="item">
	...
	if("${item.INTRO}" != 't'){
		...
		json.url = "javascript:callFunction('${item.BBS_ID}','${item.FTIME}','${item.STIME}','${item.FDATE}','${item.RDCNT}','${item.CNT}'  )";
		list.push(json); 
	}
</c:forEach>
...
function callFunction(data,ftime,stime,fdate, rdcnt , cnt ){
	if("${currDate}" > fdate ){
...
var chk="";
	$(function(){
		...
		//} else {
				...
				location.href="https://booking.naver.com/booking/12/bizes/100284/items/2618802?area=bnl";
		//}
...
<style>
.rightBox2 .bbsbtn button{margin:10px 0;margin-right;9px;}
.Learning .topBox .rightBox .btn{bottom:-22px;}
.Learning .topBox .small{padding-top:30px;}
.Learning .topBox p.small:last-of-type{padding-top:10px;}
</style>
		...
		<article class="reservation Learning">
			...
			<div class="topBox clear">
				...
				<div class="leftBox leftBox2">
                    ...
                    <div class="text_Box">
                        <h3>${vo.TITLE }새활용플라자 새활용탐방</h3>
                        <ul>
                            <li>2018년 서울 새활용플라자 정기새활용탐방 예약 프로그램입니다.</li>
                            <li>요일 : 월, 화, 수, 목</li>
				...
				<div class="rightBox rightBox1">
					<h3>${vo.TITLE }서울새활용플라자 새활용탐방_새활용 이야기</h3>
					<textarea rows="20" cols="70" readonly>${vo.SIMPLECONTENT }*새활용탐방 대상연령: 7세이상
서울새활용플라자 깊이 알기
-
...
<a id="url3d-2" target="_blank" href="/studio.do?type=Studio_Learning">
자세히 보기
</a>
					<div class="btn clear">
				...
				<p class="small">* 단체예약 전 셔틀버스 일정을 확인하시길 바랍니다</p>
				...
				<div class="contentBox3 clear">
					...
					<div class="addreBox">
						...
						<div class="btn clear bus_reservation">
						  <button class="busReservBtn">버스 예약하기</button><!--http://61.97.54.228/bookinsert.do?NTT_ID=reservbus&BBS_ID=-->
					</div>
					</div>
